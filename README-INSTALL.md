...
## D. Aktifkan Google Analytics (opsional, untuk melihat jumlah pengguna)

App ini sudah dipasangi kode Google Analytics 4 (GA4), tapi datanya baru
mengalir kalau kamu isi **Measurement ID** milikmu sendiri (gratis, ±5 menit):

1. Buka **analytics.google.com** → login dengan akun Google kamu.
2. Kalau belum punya akun Analytics, klik **Start measuring** → isi nama akun bebas.
3. Bikin **Property** baru (nama bebas, misal "Rumah Tangga App"), pilih zona
   waktu & mata uang Indonesia.
4. Saat diminta platform, pilih **Web** → isi:
   - Website URL: alamat GitHub Pages kamu, misal `https://namakamu.github.io/rumah-tangga`
   - Stream name: bebas, misal "Rumah Tangga Web"
5. Setelah stream dibuat, akan muncul **Measurement ID** berbentuk
   `G-XXXXXXXXXX`. Salin ID itu.
6. Buka file `index.html`, cari baris:
   ```js
   window.GA_MEASUREMENT_ID = 'G-XXXXXXXXXX';
   ```
   Ganti dengan Measurement ID kamu, simpan, lalu upload ulang ke GitHub
   (menimpa file lama).
7. Buka **analytics.google.com → Reports → Realtime** untuk melihat siapa
   yang sedang membuka app secara langsung, atau **Reports → Engagement**
   untuk jumlah pengguna dari waktu ke waktu (harian/mingguan/bulanan).

App ini otomatis mengirim:
- **login** — setiap kali seseorang berhasil masuk (dengan info metode:
  `google` atau `offline`, tanpa mengirim email/nama supaya tetap privat)
- **logout** — saat seseorang keluar dari akunnya
- **screen_view** — setiap kali pindah tab (Beranda, Anggaran, Menu, dst),
  jadi kamu juga bisa lihat fitur mana yang paling sering dibuka

Kalau `GA_MEASUREMENT_ID` belum diganti, app tetap jalan normal seperti biasa
— cuma datanya tidak terkirim ke mana pun.
